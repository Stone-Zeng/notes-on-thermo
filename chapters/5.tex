...
\begin{braced}[\label{eq:bose-and-fermi-dist-under-non-degenerate-condition}]
  ...
  = \frac{\Omega_\text{MB}(\qty{a_l})}{N!}.
\end{braced}
这说明在\kwd{非简并条件}式~\eqref{eq:non-degenerate-condition} 下，Bose--Einstein 分布与 Fermi--%
...
为处理方便，我们把求 $\Omega_\text{MB}$ 的极大值改为求 $\ln\Omega_\text{MB}$ 的极大值。利用
\kwd{Stirling 公式}
\begin{equation}
...
\begin{equation}
  \var{\ln\Omega_\text{MB}} - \qty(\alpha-1)\var{N} - \beta\var{E}
...
\end{align}
$a_l>0$，所以该式恒小于零。这就验证了最概然分布 $\qty{\tilde{a_l}}$ 的确对应 $\ln\Omega_\text{MB}$
的极大值。式~\eqref{eq:maxwell-boltzmann-dist} 给出的结果即为 \kwd{Maxwell--Boltzmann 分布}。

设粒子数分布相对最概然分布 $\qty{\tilde{a_l}}$ 有一微小偏离 $\var{a_l}$，并使得量子态数目变化了
$\var{\Omega_\text{MB}}$。利用 Taylor 展开，可有
\begin{align}
  \ln(\Omega_\text{MB}+\incr{\Omega_\text{MB}})
  = \ln\Omega_\text{MB}\qty(\qty{\tilde{a_l}+\var{a_l}})
  &= \ln\Omega_\text{MB}(\qty{\tilde{a_l}}) + \eval{\var{\ln\Omega_\text{MB}}}_\qty{\tilde{a_l}}
     + \frac{1}{2} \eval{\var[2]{\ln\Omega_\text{MB}}}_\qty{\tilde{a_l}} + \cdots \notag \\
  &= \ln\Omega_\text{MB}(\qty{\tilde{a_l}}) + 0
     - \frac{1}{2} \sum_l \frac{\qty(\var{a_l})^2}{\tilde{a_l}},
\end{align}
即
\begin{equation}
  \ln\frac{\Omega_\text{MB}+\incr{\Omega_\text{MB}}}{\Omega_\text{MB}}
  = -\frac{1}{2} \sum_l \qty(\frac{\var{a_l}}{\tilde{a_l}})^2 \tilde{a_l}
  \implies \frac{\Omega_\text{MB}+\incr{\Omega_\text{MB}}}{\Omega_\text{MB}}
           = \exp[-\frac{1}{2} \sum_l \qty(\frac{\var{a_l}}{\tilde{a_l}})^2 \tilde{a_l}].
\end{equation}
在热力学极限下，$\sum_l a_l=N \sim 10^{23}$，即使 $\var{a_l}$ 很小（如 $10^{-5}$），也有
\begin{equation}
  \frac{\Omega_\text{MB}+\incr{\Omega_\text{MB}}}{\Omega_\text{MB}}
  \sim \exp(-\frac{1}{2} \times 10^{-10} \times 10^{23}) \lll 1.
\end{equation}
这说明 $\Omega_\text{MB}$ 具有尖锐成锋的特性，类似数学中的 Dirac-$\delta$ 函数。

\subsection*{经典统计中 Boltzmann 分布的表达式}

之前的讨论建立在能级分立的量子情形上，现在将其推广到经典情形。我们需要在 μ 空间中定义体积元（或称为%
\kwd{相格}）$\incr{\omega_1}, \, \incr{\omega_2}, \, \cdots$。μ 空间是由坐标和动量张成一个 $2r$ 维空
间（$r$ 为自由度），其中的体积元为
\begin{equation}
  \incr{\omega}
  = \qty\big(\incr{q_1}\incr{q_2}\cdots\incr{q_r}) \, \qty\big(\incr{p_1}\incr{p_2}\cdots\incr{p_r})
  = (\incr{q_1}\incr{p_1}) \, (\incr{q_2}\incr{p_2}) \, \cdots (\incr{q_r}\incr{p_r})
  = h_0^r.
\end{equation}
式中的 $h_0=\incr{q_i}\incr{p_i}$ 为相格的尺度。相格体积越小，对运动的刻画就越精确。但量子力学为相格
的尺度给出了下限，即为 Planck 常数 $h$。

一方面，我们需将相格取得足够小，使同一相格中不同代表点所代表的粒子，其运动状态的差别可以忽略不计，即
所有代表点都具有近似相同的广义坐标 $q_i$ 和广义动量 $p_i$。另一方面，相格不能取为无穷小，因为相格中
还必须有足够数量的代表点。这种近似通常被称为 μ 空间的\kwd{粗粒近似}。在粗粒近似下，同一相格中的代表
点可认为处在同一能级 $\varepsilon_l$ 上。
